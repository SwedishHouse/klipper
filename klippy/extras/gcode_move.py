...
class THCHandler:
    ...
    def count_segments(line_length, segment_length):
        ...
        return ceil(line_length / segment_length)
    ...
    def __printer_ready_event(self):
        """
        Lookup for pin up and pin down events
        It will happen when printer will be ready
        :return: None
        """
        pass
    ...
    def update_position_in_plane(self, coord: dict):
        ...
        if 'Y' in coord:    
            self._position_in_the_plane['Y'] = coord['Y']
        if 'F' in coord:    
            self._position_in_the_plane['F'] = coord['F']
        

    def parse_gcode(self, gcode):

        g_cmd = {'G': 0, 'X': 0, 'Y': 0, 'Z': 0, 'F': 0}
        match = re.search(self.pattern, gcode)
        if match:
            g_cmd['G'] = match.group('G')
            g_cmd['X'] = float(match.group('X'))
            g_cmd['Y'] = float(match.group('Y'))
            g_cmd['Z'] = float(match.group('Z'))
            g_cmd['F'] = float(match.group('F'))
        return g_cmd

    def new_distance(self, z_value, feed_rate):
        pass
    ...
    def __next__(self):
        if self.__index < len(self.__segments):
            item = self.__segments[self.__index]
            self.__index += 1
            return item
        else:
            raise StopIteration
